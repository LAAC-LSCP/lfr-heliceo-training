#Readme
##Formation LFR — HéLiCéO 🎙️

Bienvenue dans le dépôt de formation du projet **HéLiCéO** (Héritages Linguistiques, Cultures orales, Éducation en Océanie — CNRS, 2025–2029).
Durant ces deux jours de formation, nous allons explorer ensemble l'ensemble du processus de collecte de données : des enregistrements sur le terrain jusqu'à l'analyse automatisée. Ce dépôt est votre compagnon de route — il contient toutes les guides et ressources pédagogiques dont vous aurez besoin **pendant la formation, sur le terrain, et après votre retour**. Gardez-le à portée de main !

---

## 🎯 Objectif de la formation

Vous préparer à conduire des enregistrements audio et vidéo naturalistes de longue durée (Long-Form Recordings) dans des communautés plurilingues de Polynésie française, dans le respect des protocoles éthiques et des bonnes pratiques de gestion des données.

---

## 📅 Structure de la formation

La formation se déroule sur **2 jours (8 heures au total)** :

- **Jour 1** — Présentation du projet, collecte de données, éthique et consentement, introduction à la gestion des données
- **Jour 2** — Pratique complète du workflow : enregistrement → transfert → nommage des fichiers → annotation → analyse automatisée

→ Consulter le programme complet : [**Structure de la formation**](./01_structure_formation.md)

---

## 📚 Contenu du dépôt
   # | Document | Description |
 |---|----------|-------------|
 | 1 | [Structure de la formation](./01_structure_formation.md) | Programme des 2 jours, objectifs et approche pédagogique |
 | 2 | [Objectifs scientifiques](./02_objectifs_scientifiques.md) | Objectifs scientifiques des enregistrements audio et du dispositif vidéo (harnais-caméra) |
 | 3 | [Guide Izyrec — pas à pas](./03_guide_izyrec.md) | Comment préparer, utiliser et entretenir l'enregistreur audio Izyrec |
 | 4 | [FAQ — Communiquer avec les participants](./05_faq_participants.md) | Réponses aux questions fréquentes sur le consentement, l'éthique et la communication avec les familles et les communautés 
 | 5 | [Gestion des données](./06_gestion_donnees.md) | Nommage des fichiers, structure des dossiers, métadonnées et transfert |
 | 6 | [Analyse automatisée avec ELSI](./08_elsi.md) | Comment utiliser ELSI pour analyser vos enregistrements et interpréter les résultats |

---

## ❓ Questions

Si vous avez des questions sur les matériaux ou le déroulement de la formation, n'hésitez pas à contacter l'équipe ExELang & HéLiCéO.

---

*Dépôt maintenu par l'équipe ExELang & HéLiCéO — LSCP, École Normale Supérieure*
